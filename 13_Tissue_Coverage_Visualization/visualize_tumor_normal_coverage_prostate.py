#!/usr/bin/env python
"""
visualize_tumor_normal_coverage.py  (Prostate)
...
the two tissue types present in prostate cancer data: Tumor and Benign.

Expected input columns (wide format, one row per model-percentile pair):
    model, percentile,
    mean_tumor_cov, std_tumor_cov, n_valid_tumor_cov,
    mean_benign_cov, std_benign_cov, n_valid_benign_cov,
...
Usage:
    python visualize_tumor_normal_coverage.py \\
        ...
        --output prostate_tumor_benign.png
"""
...
TISSUES = [
    ...
    ("Benign", "mean_benign_cov", "std_benign_cov", "n_valid_benign_cov"),
]
...
TISSUES_PCT_ATTN = [
    ...
    ("Benign", "mean_pct_attn_in_benign", "std_pct_attn_in_benign", "n_valid_pct_attn_in_benign"),
]

REQUIRED_COLUMNS = [
    "model", "percentile",
    "mean_tumor_cov", "mean_benign_cov",
    "mean_pct_attn_in_tumor", "mean_pct_attn_in_benign",
]
...
def parse_args():
    p = argparse.ArgumentParser(description="Visualize tumor/benign coverage for prostate data.")
    ...
    p.add_argument("--output", type=str, default=None,
                   help="Output path for figure (default: same dir as CSV, "
                        "prostate_tumor_benign_coverage.png)")
...
def main():
    ...
    tissues = [t[0] for t in TISSUES]  # ["Tumor", "Benign"]

    if args.output is None:
        out_path = csv_path.parent / "prostate_tumor_benign_coverage.png"
    else:
    ...
    # 4 columns: coverage Tumor, coverage Benign, % in attn Tumor, % in attn Benign
    ...
    plt.suptitle("Prostate Cancer — Tumor vs Benign Coverage by Model", fontsize=13, y=1.01)
    ...
    # ---- Double-bar figures: one per percentile (tumor up red, benign down green) ----
    ...
    for p in percentiles:
        ...
        benign_ser = sub[sub["tissue"] == "Benign"].set_index("model")["mean_coverage"]
        tumor_vals = np.array([tumor_ser.get(m, np.nan) for m in model_order])
        benign_vals = np.array([benign_ser.get(m, np.nan) for m in model_order])
        tumor_vals = np.nan_to_num(tumor_vals, nan=0.0)
        benign_vals = np.nan_to_num(benign_vals, nan=0.0)

        use_broken = benign_vals.size == 0 or benign_vals.max() < 20
        ...
        if use_broken:
            y_down = _broken_axis_transform(benign_vals)
            ax.bar(x_pos, y_down, bar_width, bottom=0, color="#00FF00", edgecolor="black", linewidth=0.8, label="Benign")
        else:
            ax.bar(x_pos, -benign_vals, bar_width, bottom=0, color="#00FF00", edgecolor="black", linewidth=0.8, label="Benign")
        ax.axhline(0, color="black", linewidth=0.8)
        ax.set_ylim(-100, 100)
        ax.set_xticks(x_pos)
        ax.set_xticklabels([])
        # Single left axis: 100 at top, 0 at center, 100 at bottom; two labels (tumor top, benign bottom)
        ...
        ax.text(-0.18, 0.22, "% Benign coverage", rotation=90, transform=ax.transAxes, fontsize=10, va="center", ha="center")
