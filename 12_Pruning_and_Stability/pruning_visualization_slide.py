...
2. Horizontal box plots showing F1 score distributions for pruned slide-level models

This script:
1. Reads pruning data from an Excel file with feature counts at different correlation thresholds
2. Loads pruned slide-level MLP models and generates F1 score distributions using bootstrap sampling
...
Example:
  python pruning_visualization_slide.py \
    --pruning_excel /path/to/slide_pruning.xlsx \
    --pruned_model_dirs /path/to/kidney_slide_combpruned01_s1 /path/to/kidney_slide_combpruned02_s1 ... \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Create pruning visualization with thermometer plots and F1 score box plots for slide-level models"
    )
    parser.add_argument(
        "--pruning_excel", type=str, required=True,
        help="Excel file containing pruning data with feature counts at different thresholds (slide_pruning.xlsx)"
    )
    parser.add_argument(
        "--pruned_model_dirs", type=str, nargs='+', required=True,
        help="List of directories containing pruned slide-level models (thresholds 0.1-0.7)"
    )
    parser.add_argument(
...
def extract_threshold_from_model_dir(model_dir):
    ...
    # Expected format for slide-level models: simple 2-digit number (e.g., "01", "04")
    # where "01" means 0.1, "04" means 0.4, etc.
    match = re.search(r'^(\d{2})$', model_name)
    if match:
        threshold_int = int(match.group(1))
        threshold_float = round(threshold_int * 0.1, 1)  # Convert 01->0.1, 04->0.4, etc. and round to 1 decimal
        return threshold_float
    
    # Fallback: Try to match the tile-level format: {disease}_slide_combpruned0{threshold}_s1
...
def main():
    ...
    print("="*80)
    print("Slide-level Model Pruning Visualization with Thermometer Plots and F1 Score Box Plots")
    print("="*80)
    ...
    print("\nDEBUG - Received model directories:")
    for i, mdir in enumerate(args.pruned_model_dirs):
        print(f"  [{i}] {mdir}")
    print("\nDEBUG - Received feature directories:")
    for i, fdir in enumerate(args.pruned_feature_dirs):
        print(f"  [{i}] {fdir}")
    ...
    for threshold_str, feature_dir in feature_dirs_by_threshold.items():
        ...
        if sample_files:
            ...
            embed_dims[threshold_str] = load_feature_vector(sample_path).shape[0]
    ...
    for threshold_str, model_dir in model_dirs_by_threshold.items():
        best_folds[threshold_str] = select_best_fold_for_model(model_dir)
    ...
    for threshold_str, model_dir in model_dirs_by_threshold.items():
        ...
        model = load_model_for_fold(model_dir, best_folds[threshold_str], 
                                   embed_dims[threshold_str], device)
        ...
        predictions, probabilities = generate_predictions_for_model(
            model, test_slides, feature_dir, device, f"threshold_{threshold_str}"
        )
    ...
    # Add overall title
    fig.suptitle('Slide-level Feature Pruning Analysis: Retention vs Performance', fontsize=16, fontweight='bold')
    
    # Adjust layout
    plt.tight_layout()
    
    # Save the combined plot
    output_path = os.path.join(args.output_dir, 'pruning_visualization_slide.png')
